...
from connPFM.connectivity import ev, utils
...
def test_calculate_ets(ets_auc_original_file, AUC_file, atlas_file):
    ...
    ets_auc_calc, u_vec, v_vec = utils.calculate_ets(AUC_img, AUC_img.shape[1])
...
def test_rss_surr(AUC_file, atlas_file, surr_dir, rssr_auc_file):
    ...
    AUC_img = masker.fit_transform(AUC_file)
    _, u, v = utils.calculate_ets(AUC_img, AUC_img.shape[1])
    rssr, _, _, _ = utils.rss_surr(AUC_img, u, v, join(surr_dir, "surrogate_AUC_"), "", masker, 0)
...
def test_threshold_ets_matrix():
    ...
    dum_mat2[2, 1] = 3
    th_dum = utils.threshold_ets_matrix(dum_mat, [2], 2)
